...
class Config:
    ...
    def get_modules():
        ...
        modules = {}
        entries = os.scandir('./config/available/')
    ...
    def get_thing(thing):
        with open('./config/available/' + thing + '.json', 'r') as file:
            config = file.read().replace('\n', "")
    ...
    def delete_thing(thing):
        try:
            os.remove('./config/available/' + thing + '.json')
        except FileNotFoundError:
            pass
        return os.path.exists('./config/available/' + thing + '.json')

    @staticmethod
    def put_thing(thing, data):
        with open('./config/available/' + thing + '.json', 'w') as file:
    ...
    def get_enabled():
        ...
        config['available'] = [x.split('.')[0] for x in os.listdir('./config/available/')]
